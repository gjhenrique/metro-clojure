...
(defn- valid-connection
  ...
  (let [line-name (:name line-config)
    ...
    (or (add-connections graph connections)
        ...
        (connections-removing-cycle graph (:stations line-config) line-name))))
...
(defn traverse-graph-with-successors
  "Add a new case of checking successors"
  [state]
  (let [{:keys [graph current-node current-line]} state
        predecessor (find-unvisited-predecessor graph current-node)
    ...
    (cond
      (not (nil? predecessor))
      (traverse-graph-with-successors (assoc state :current-node predecessor))

      (and (metro.algorithm/visited? graph current-node) (seq successors))
      (traverse-graph-with-successors (assoc state :current-node (first successors)))
...
(def state1 (metro.blog/traverse-graph-with-successors {:graph g :current-node "B"})) 
(def state2 (metro.blog/traverse-graph-with-successors state1)) 
(def state3 (metro.blog/traverse-graph-with-successors state2)) 
